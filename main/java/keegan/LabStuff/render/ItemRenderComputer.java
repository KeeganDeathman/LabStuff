...
public class ItemRenderComputer implements IItemRenderer {

	protected ModelBlockComputer computerModel;
	protected ResourceLocation Tex;
	
	
	public ItemRenderComputer()
	{
		computerModel = new ModelBlockComputer();
		Tex = new ResourceLocation("labstuff:textures/models/BlockComputer.png");
	}
	...
	private void renderEquipped(boolean inventory, ItemRenderType type,  Object... data)
    {
        ...
        if(inventory)
        {
        	...
        	computerModel.render(null, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.03125F);
        }
        else if(!inventory)
        {
    		Minecraft.getMinecraft().renderEngine.bindTexture(Tex);
        	computerModel.render((Entity) data[1], 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.03125F);
        }
