...
public class BlockWorkBench extends Block implements ITileEntityProvider
{
	...
	public BlockWorkBench(Material materialIn) {
		super(materialIn);
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack held, EnumFacing facing, float fx, float par8, float par9) {
		...
		if(FMLCommonHandler.instance().getEffectiveSide().isServer())
		{
			...
			player.addChatMessage(new TextComponentString("Network is holding " + tileEntity.getEnergy() + " LV"));
			if (!player.isSneaking()) {
				player.openGui(LabStuffMain.instance, 18, world, pos.getX(), pos.getY(), pos.getZ());
				return true;
			}
		}
		return false;
