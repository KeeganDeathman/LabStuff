...
public class StatsCapability extends LSPlayerStats
{
    ...
    public void loadNBTData(NBTTagCompound nbt)
    {
        try
        {
            ...
            if (p != null)
            {
                ItemStack[] saveinv = CommandLSInv.getSaveData(p.getGameProfile().getName().toLowerCase());
                if (saveinv != null)
                {
                    CommandLSInv.doLoad(p);
                }
            }
            ...
            else
            {
                ...
                this.launchpadStack = new ItemStack(LabStuffMain.landingPad, 9, 0);
            }
        ...
        {
            LSLog.severe("Found error in saved Galacticraft player data for " + player.get().getGameProfile().getName() + " - this should fix itself next relog.");
            e.printStackTrace();
        }

        LSLog.debug("Finished loading GC player data for " + player.get().getGameProfile().getName() + " : " + this.buildFlags);
    }

    @Override
    public void copyFrom(LSPlayerStats oldData, boolean keepInv)
    {
        if (keepInv)
        {
