...
import keegan.labstuff.client.IScreenManager;
...
public class DrawGameScreen extends IScreenManager
{
    ...
    {
        if (this.mapDone || reusableMap == null || LabStuffUtils.getDimensionID(this.driver.getWorld()) != 0)
        {
            return;
        }
    ...
    {
        if (type >= LabStuffRegistry.getMaxScreenTypes())
        {
    ...
    private void doDraw(int type, float ticks)
    {
        ...
        LabStuffRegistry.getGameScreen(type).render(type, ticks, scaleX, scaleZ, this);
