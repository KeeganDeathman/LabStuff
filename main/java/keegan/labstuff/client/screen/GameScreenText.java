...
import keegan.labstuff.entities.ITelemetry;
...
public class GameScreenText implements IGameScreen
{
    ...
    public void render(int type, float ticks, float sizeX, float sizeY, IScreenManager scr)
    {
        ...
        String strName = "";
        String[] str = { LabStuffUtils.translate("gui.display.nolink"), "", "", "", "" };
        ...
        if (telemeter != null && telemeter.clientData.length >= 3)
        {
            if (telemeter.clientClass != null)
            {
                ...
                {
                    ((EntitySkeleton) entity).func_189768_a(SkeletonType.values()[telemeter.clientData[3]]);
                }
...
//                    ((EntityZombie) entity).setVillager(telemeter.clientData[3] == 1); TODO Fix for MC 1.10
            ...
            else if (entity instanceof EntityLivingBase)
            {
                ...
                str[0] = telemeter.clientData[0] > 0 ? LabStuffUtils.translate("gui.player.ouch") : "";
                if (telemeter.clientData[1] >= 0)
                {
                    str[1] = LabStuffUtils.translate("gui.player.health") + ": " + telemeter.clientData[1] + "%";
                }
                else
                {
                    str[1] = "";
                }
                str[2] = "" + telemeter.clientData[2] + " " + LabStuffUtils.translate("gui.player.bpm");
                if (telemeter.clientData[3] > -1)
                {
                    str[3] = LabStuffUtils.translate("gui.player.food") + ": " + telemeter.clientData[3] + "%";
                }
                if (telemeter.clientData[4] > -1)
                {
                    ...
                    {
                        str[4] = LabStuffUtils.translate("gui.oxygen_storage.desc.1") + ": OK";
                    }
                    else
                    {
                        str[4] = LabStuffUtils.translate("gui.oxygen_storage.desc.1") + ": " + this.makeOxygenString(oxygen) + LabStuffUtils.translate("gui.seconds");
                    }
                }
            }
            else
...
//            RenderPlayerGC.flagThermalOverride = false;
    ...
    public static String makeSpeedString(int speed100)
    {
        ...
        String spstr1 = LabStuffUtils.translate("gui.rocket.speed") + ": " + sp1;
        String spstr2 = (sp2 > 9 ? "" : "0") + sp2;
        return spstr1 + "." + spstr2 + " " + LabStuffUtils.translate("gui.lander.velocityu");
    }
