...
import keegan.labstuff.items.ItemOxygenTank;
...
public class ContainerExtendedInventory extends Container
{
    ...
    public ContainerExtendedInventory(EntityPlayer thePlayer, InventoryExtended extendedInventory)
    {
        ...
        for (i = 0; i < 4; ++i)
        {
            this.addSlotToContainer(new SlotArmorLS(thePlayer, thePlayer.inventory, 39 - i, 61, 8 + i * 18, i));
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par1)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            else
            {
                ...
                for (int j = 36; j < 40; j++)
                {
                    if (((SlotArmorLS) this.inventorySlots.get(j)).isItemValid(stack))
                    {
