...
public class MapGen extends BiomeProvider implements Runnable
{
    ...
    {
        this.dimID = LabStuffUtils.getDimensionID(world);
        ...
        GenLayer[] agenlayerOrig = GenLayerLSMap.initializeAllBiomeGenerators(seed, worldType, world.getWorldInfo().getGeneratorOptions());
        GenLayer[] agenlayer;
        try {
        ...
        {
            LSLog.severe("LabStuff background map image generator not able to run (probably a mod conflict?)");
            LSLog.severe("Please report this at https://github.com/micdoodle8/Galacticraft/issues/2481");
            e.printStackTrace();
            this.mapNeedsCalculating = false;
            return;
        }
        ...
        LSLog.debug("Starting map generation " + file.getName() + " top left " + ((biomeMapCx - limitX) * 16) + "," + ((biomeMapCz - limitZ) * 16));
    ...
    private void flagProgress()
    {
        ...
        LSLog.debug("Saving partial map image progress " + progX);
