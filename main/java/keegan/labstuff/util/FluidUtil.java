...
public class FluidUtil
{
    ...
    public static boolean isFuelContainerAny(ItemStack var4)
    {
        ...
        {
            return var4.getItem() == LabStuffMain.fuelCanister && var4.getItemDamage() < var4.getMaxDamage();
        }
    ...
    /**
     * Fill LabStuff entities (e.g. rockets, buggies) with LabStuff fuel.
     ...
     */
    public static int fillWithLSFuel(FluidTank tank, FluidStack liquid, boolean doFill)
    {
        if (liquid != null && testFuel(FluidRegistry.getFluidName(liquid)))
        {
            final FluidStack liquidInTank = tank.getFluid();

            //If the tank is empty, fill it with the current type of LS fuel
            if (liquidInTank == null)
            {
                return tank.fill(new FluidStack(LabStuffMain.kerosene, liquid.amount), doFill);
            }
    ...
    public static void tryFillContainerFuel(FluidTank tank, ItemStack[] inventory, int slot)
    {
        if (FluidUtil.isValidContainer(inventory[slot]))
        {
            ...
            if (liquid != null && liquid.amount > 0)
            {
                ...
                if (liquidname.startsWith("kerosene"))
                {
                    //Make sure it is the current LS fuel
                    if (!liquidname.equals(LabStuffMain.kerosene.getName()))
                    {
                        liquid = new FluidStack(LabStuffMain.kerosene, liquid.amount);
                    }
                    ...
                    if (stack.getItem() instanceof IFluidContainerItem)
                    {
                        ...
                        if (existingFluid != null && !existingFluid.getFluid().getName().equals(LabStuffMain.kerosene.getName()))
                        {
                            liquid = new FluidStack(existingFluid, liquid.amount);
                        }
                    }

                    FluidUtil.tryFillContainer(tank, liquid, inventory, slot, LabStuffMain.fuelCanister);
                }
            }
        }
    }
     ...
     * It can be either an empty container, or a LabStuff canister
     ...
     * Either LabStuff canisters or Forge containers
     *
     * @param var4
     * @param canisterType
     * @return
	...
	public static int fillWithLSGas(FluidTank tank, FluidStack liquid, boolean doFill) 
	{
        if (liquid != null && testFuel(FluidRegistry.getFluidName(liquid)))
        {
            final FluidStack liquidInTank = tank.getFluid();
            ...
            if (liquidInTank == null)
            {
                return tank.fill(new FluidStack(LabStuffMain.bioDiesel, liquid.amount), doFill);
            }
