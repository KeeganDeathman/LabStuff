...
public class OxygenUtil
{
    ...
    {
        checked.add(pos);
        if (block == LabStuffMain.breatheableAir || block == LabStuffMain.brightBreatheableAir)
        {
    ...
    public static boolean noAtmosphericCombustion(WorldProvider provider)
    {
        ...
        {
            return (!((ILabstuffWorldProvider) provider).isGasPresent(IAtmosphericGas.OXYGEN) && !((ILabstuffWorldProvider) provider).hasBreathableAtmosphere());
        }
    ...
    {
        int dimID = LabStuffUtils.getDimensionID(worldObj);
