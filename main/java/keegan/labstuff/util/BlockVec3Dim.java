...
public class BlockVec3Dim implements Cloneable
{
    ...
    public Block getBlockID()
    {
        ...
        try
        {
            ...
            {
                return BlockVec3Dim.chunkCached.getBlockState(this.x & 15, this.y, this.z & 15).getBlock();
            }
            else
            {
                Chunk chunk = null;
                ...
                return chunk.getBlockState(this.x & 15, this.y, this.z & 15).getBlock();
            }
        }
    ...
    public Block getBlockID_noChunkLoad()
    {
        ...
        try
        {
            if (world.getChunkProvider().provideChunk(chunkx, chunkz) != null)
            {
                // In a typical inner loop, 80% of the time consecutive calls to
                // this will be within the same chunk
                if (BlockVec3Dim.chunkCacheX == chunkx && BlockVec3Dim.chunkCacheZ == chunkz && BlockVec3Dim.chunkCacheDim == world.provider.getDimension() && BlockVec3Dim.chunkCached.isLoaded())
    ...
    {
   		World world = LabStuffMain.proxy.getWorldForID(this.dim);
   		if (world == null) return null;
