...
public class ColorUtil
{
    ...
    public static void sendUpdatedColorsToPlayer(LSPlayerStats stats)
    {
        int dimID = stats.getPlayer().get().dimension;
        LabStuffMain.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_RECOLOR_ALL_GLASS, dimID, new Object[] { Integer.valueOf(stats.getGlassColor1()), Integer.valueOf(stats.getGlassColor2()), Integer.valueOf(stats.getGlassColor3()) }), stats.getPlayer().get());
    }
    
    public static void updateColorsForArea(int dimID, BlockPos pos, int range, int color1, int color2, int color3)
    {
        LabStuffMain.packetPipeline.sendToAllAround(new PacketSimple(EnumSimplePacket.C_RECOLOR_ALL_GLASS, dimID, new Object[] { color1, color2, color3 }), new TargetPoint(dimID, pos.getX(), pos.getY(), pos.getZ(), range));
    }
    ...
    {
        int changes = 0;
        changes += LabStuffMain.spaceGlassVanilla.setColor(color1);
        changes += LabStuffMain.spaceGlassClear.setColor(color2);
        changes += LabStuffMain.spaceGlassStrong.setColor(color3);
