...
import keegan.labstuff.client.screen.DrawGameScreen;
...
public class MapUtil
{
    ...
    {
        LabStuffClientProxy.overworldTexturesValid = false;
        ...
        LabStuffMain.packetPipeline.sendToServer(new PacketSimple(PacketSimple.EnumSimplePacket.S_REQUEST_OVERWORLD_IMAGE, LabStuffUtils.getDimensionID(FMLClientHandler.instance().getClient().theWorld), new Object[] {}));
    ...
    public static void makeOverworldTexture()
    {
        ...
        World overworld = WorldUtil.getWorldForDimensionServer(ConfigManagerCore.idDimensionOverworld);
        if (overworld == null)
        {
            return;
        }
        ...
        File baseFolder = new File(overworld.getMinecraftServer().worldServerForDimension(0).getChunkSaveLocation(), "labstuff/overworldMap");
    ...
    public static void sendOverworldToClient(EntityPlayerMP client)
    {
        if (doneOverworldTexture)
        {
            try
            {
                File baseFolder = new File(client.worldObj.getMinecraftServer().worldServerForDimension(0).getChunkSaveLocation(), "labstuff/overworldMap");
                if (!baseFolder.exists())
                {
                    LSLog.severe("Base folder missing: " + baseFolder.getAbsolutePath());
                    return;
                }
    ...
    private static void sendMapPacketCompressed(int cx, int cz, EntityPlayerMP client, byte[] map) throws IOException
    {
        if (cx == LARGEMAP_MARKER && map.length < 2040000)
        {
            ...
            LabStuffMain.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_SEND_OVERWORLD_IMAGE, LabStuffUtils.getDimensionID(client.worldObj), new Object[] { cx, map.length, largeMapPartA }), client);
            LabStuffMain.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_SEND_OVERWORLD_IMAGE, LabStuffUtils.getDimensionID(client.worldObj), new Object[] { cx + 1, map.length, largeMapPartB }), client);
        }
        else if (map.length < 1020000)  //That's about the limit on a Forge packet length
    	{
    		LabStuffMain.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_SEND_OVERWORLD_IMAGE, LabStuffUtils.getDimensionID(client.worldObj), new Object[] { cx, cz, map }), client);
    	}
    }
    ...
    private static void sendMapPacketAllCompressed(int cx, int cz, byte[] map)
    {
        if (cx == LARGEMAP_MARKER && map.length < 2040000)
        {
            ...
            byte[] largeMapPartA = Arrays.copyOf(map, halfSize);
            byte[] largeMapPartB = Arrays.copyOfRange(map, halfSize, map.length);
            LabStuffUtils.sendToAllDimensions(EnumSimplePacket.C_SEND_OVERWORLD_IMAGE, new Object[] { cx, map.length, largeMapPartA });
            LabStuffUtils.sendToAllDimensions(EnumSimplePacket.C_SEND_OVERWORLD_IMAGE, new Object[] { cx + 1, map.length, largeMapPartB });
        }
        else if (map.length < 1020000)  //That's about the limit on a Forge packet length
        {
            LabStuffUtils.sendToAllDimensions(EnumSimplePacket.C_SEND_OVERWORLD_IMAGE, new Object[] { cx, cz, map });
        }
    }
...
//	        		slowMap.pause();
    ...
    public static void writeImgToFile(BufferedImage img, String name)
    {
        if (LabStuffMain.enableJPEG)
        {
            ...
            try
            {
                ...
                LabStuffMain.jpgWriter.setOutput(outputStreamA);
                LabStuffMain.jpgWriter.write(null, new IIOImage(img, null, null), LabStuffMain.writeParam);
                outputStreamA.close();
            }
            catch (Exception e)
            {
    ...
    public static void receiveOverworldImageCompressed(int cx, int cz, byte[] raw) throws IOException
    {
        ...
        {  
            LSLog.debug(e.toString());
            LSLog.debug("Client received a corrupted map image data packet from server " + cx + "_" + cz);
        }
    }
...
//            if (LabStuffClientProxy.overworldTextureLarge == null)
//            {
//                LabStuffClientProxy.overworldTextureLarge = new DynamicTextureProper(WIDTH_WORLD * 2, HEIGHT_WORLD * 2);
//            }
//            LabStuffClientProxy.overworldTextureLarge.update(worldImageLarge);

            //Write it to a .jpg file on client for beta preview 
            if (LabStuffMain.enableJPEG && folder != null)
            {
                ImageOutputStream outputStream = new FileImageOutputStream(new File(folder, "large.jpg"));
                LabStuffMain.jpgWriter.setOutput(outputStream);
                LabStuffMain.jpgWriter.write(null, new IIOImage(worldImageLarge, null, null), LabStuffMain.writeParam);
                outputStream.close();
            }
        }
        ...
        else if (raw.length == OVERWORLD_TEXTURE_WIDTH * OVERWORLD_TEXTURE_HEIGHT * 2)
        {
            ...
            try
            {
                InputStream in = rm.getResource(new ResourceLocation("labstuff:textures/gui/celestialbodies/earth.png")).getInputStream();
            ...
            if (result != null)
            {
                if (LabStuffClientProxy.overworldTextureWide == null)
                {
                    LabStuffClientProxy.overworldTextureWide = new DynamicTextureProper(OVERWORLD_TEXTURE_WIDTH, OVERWORLD_TEXTURE_HEIGHT);
                }
                if (LabStuffClientProxy.overworldTextureClient == null)
                {
                    LabStuffClientProxy.overworldTextureClient = new DynamicTextureProper(OVERWORLD_TEXTURE_HEIGHT, OVERWORLD_TEXTURE_HEIGHT);
                }
                LabStuffClientProxy.overworldTextureWide.update(result);
                LabStuffClientProxy.overworldTextureClient.update(result);
                LabStuffClientProxy.overworldTexturesValid = true;
            }
        }
        else if (folder != null)
        {
    ...
    public static boolean getMap(int[] image, World world, BlockPos pos)
    {
        ...
        int dim = LabStuffUtils.getDimensionID(world);
        boolean result = true;
    ...
    private static boolean makeRGBimage(int[] array, File baseFolder, int cx, int cz, int offsetX, int offsetZ, int xCoord, int zCoord, int dim, boolean prevResult)
    {
        ...
        if (!filename.exists())
        {
            ...
            {
                //LSLog.debug("Info: Server not yet ready to send map file " + baseFolder.getName() + "/" + filename.getName());
            }
            else
            {
                clientRequests.add(filename.getName());
                //LSLog.debug("Info: Client requested map file" + filename.getName());
                LabStuffMain.packetPipeline.sendToServer(new PacketSimple(PacketSimple.EnumSimplePacket.S_REQUEST_MAP_IMAGE, LabStuffUtils.getDimensionID(FMLClientHandler.instance().getClient().theWorld), new Object[] { dim, cx, cz }));
            }
            return true;
        }
        ...
        {
            LSLog.severe("Problem reading map file: " + baseFolder.getAbsolutePath() + filename.getName());
            return true;
        }
        if (raw == null || raw.length != SIZE_STD * SIZE_STD * 2)
        {
            LSLog.debug("Warning: unexpected map size is " + raw.length + " for file " + filename.toString());
            return true;
        }
        ...
        for (int x = xstart; x < SIZE_STD; x++)
        {
            ...
            for (int z = zstart; z < SIZE_STD; z++)
            {
                ...
                {
                    LSLog.debug("Outside image " + imagex + "," + imageZ + " - " + "x=" + x + " z=" + z + " offsetX=" + offsetX + " offsetZ = " + offsetZ + " ox=" + ox + " oz=" + oz);
                }
                else
                {
