...
public class ItemBuggy extends Item implements IHoldableItem, ISortableItem
{
        ...
        else
        {
            ...
            else
            {
                if (var24.typeOfHit == RayTraceResult.Type.BLOCK)
                {
                    ...
                    {
                        var35.buggyFuelTank.setFluid(new FluidStack(LabStuffMain.bioDiesel, itemStackIn.getTagCompound().getInteger("BuggyFuel")));
                    }
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer player, List<String> tooltip, boolean b)
    {
        ...
        {
            tooltip.add(LabStuffUtils.translate("gui.buggy.storage_space") + ": " + par1ItemStack.getItemDamage() * 18);
        }

        if (par1ItemStack.hasTagCompound() && par1ItemStack.getTagCompound().hasKey("BuggyFuel"))
        {
            tooltip.add(LabStuffUtils.translate("gui.message.fuel.name") + ": " + par1ItemStack.getTagCompound().getInteger("BuggyFuel") + " / " + EntityBuggy.tankCapacity);
        }
    }
