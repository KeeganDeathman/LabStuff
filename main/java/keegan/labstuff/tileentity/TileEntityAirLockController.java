...
public class TileEntityAirLockController extends TileEntityAirLock
{
    ...
    public void sealAirLock()
    {
        ...
        if (this.worldObj.getBlockState(new BlockPos(x, y, z)).getBlock() != LabStuffMain.airLockSeal)
        {
            this.worldObj.playSound(null, x, y, z, LSSounds.openAirLock, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
            ...
            if (this.protocol.minY == this.protocol.maxY && this.protocol.minX != this.protocol.maxX && this.protocol.minZ != this.protocol.maxZ)
            {
                for (x = this.protocol.minX + 1; x <= this.protocol.maxX - 1; x++)
                {
                    for (z = this.protocol.minZ + 1; z <= this.protocol.maxZ - 1; z++)
                    {
                        ...
                        {
                            this.worldObj.setBlockState(pos, LabStuffMain.airLockSeal.getDefaultState(), 3);
                        }
                    }
                }
            }
    ...
    public void unsealAirLock()
    {
        ...
        {
            this.worldObj.playSound(null, x, y, z, LSSounds.closeAirLock, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
            ...
            if (this.protocol.minY == this.protocol.maxY && this.protocol.minX != this.protocol.maxX && this.protocol.minZ != this.protocol.maxZ)
            {
                for (x = this.protocol.minX + 1; x <= this.protocol.maxX - 1; x++)
                {
                    for (z = this.protocol.minZ + 1; z <= this.protocol.maxZ - 1; z++)
                    {
                        ...
                        if (this.worldObj.getBlockState(pos).getBlock() == LabStuffMain.airLockSeal)
                        {
