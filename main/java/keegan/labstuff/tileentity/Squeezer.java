...
public class Squeezer extends TileEntity implements IEnergyWrapper, IFluidHandlerWrapper, ITickable
{
	...
	public void update()
	{
		if(outputI == null && outputF == null)
		{
			...
			for(Squeeze squeeze : Recipes.squeezes)
			{
				if(getStackInSlot(0) != null && squeeze.getInput() != null && ItemStack.areItemsEqual(getStackInSlot(0), squeeze.getInput()))
				{
					if(buffer >= 100)
					{
						buffer -= 100;
						...
						outputI = squeeze.getItemOut();
						outputF = squeeze.getFluidOut();
					}
				}
			}
