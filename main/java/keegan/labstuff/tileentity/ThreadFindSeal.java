...
import keegan.labstuff.wrappers.ScheduledBlockChange;
...
//            }
    ...
    public class ThreadedFindSeal extends Thread
    {
        ...
        {
            super("LS Sealer Roomfinder Thread");
    ...
    public void check()
    {
        ...
        if (this.checkCount > 0)
        {
            ...
            if (this.head.x < -29990000 || this.head.z < -29990000 || this.head.x >= 29990000 || this.head.z >= 29990000)
            {
                ...
                else if (state.getBlock() == LabStuffMain.brightAir)
                {
            ...
            else
            {
                ...
                else if (headState.getBlock() == LabStuffMain.brightAir)
                {
                    this.airToReplaceBright.add(this.head.clone());
                }
                this.doLayer();
        ...
        else
        {
            ...
            if (!this.sealed)
            {
                ...
                if (block == LabStuffMain.breatheableAir)
                {
                    this.breatheableToReplace.add(this.head);
                }
                if (block == LabStuffMain.brightBreatheableAir)
                {
    ...
    private void makeSealGood(boolean ambientThermal)
    {
        if (!this.airToReplace.isEmpty() || !this.airToReplaceBright.isEmpty() || !ambientThermalTracked.isEmpty() || !ambientThermalTracked.isEmpty())
        {
            ...
            Block breatheableAirID = LabStuffMain.breatheableAir;
            Block breatheableAirIDBright = LabStuffMain.brightBreatheableAir;
            ...
            TickHandlerServer.scheduleNewBlockChange(LabStuffUtils.getDimensionID(this.world), changeList);
        }
        if (!this.torchesToUpdate.isEmpty())
        {
            TickHandlerServer.scheduleNewTorchUpdate(LabStuffUtils.getDimensionID(this.world), this.torchesToUpdate);
        }
    }
    ...
    private void makeSealBad()
    {
        if (!this.breatheableToReplace.isEmpty() || !this.breatheableToReplaceBright.isEmpty())
        {
            ...
            {
                changeList.add(new ScheduledBlockChange(checkedVec.toBlockPos(), LabStuffMain.brightAir.getDefaultState(), 0));
            }
            TickHandlerServer.scheduleNewBlockChange(LabStuffUtils.getDimensionID(this.world), changeList);
        }
        if (!this.torchesToUpdate.isEmpty())
    ...
    private void unseal()
    {
        ...
        Block breatheableAirID = LabStuffMain.breatheableAir;
        Block breatheableAirIDBright = LabStuffMain.brightBreatheableAir;
        Block oxygenSealerID = LabStuffMain.oxygenSealer;
        Block fireBlock = Blocks.FIRE;
        Block airBlock = Blocks.AIR;
        Block airBlockBright = LabStuffMain.brightAir;
    ...
    private void doLayer()
    {
        ...
        Block breatheableAirIDBright = LabStuffMain.brightBreatheableAir;
        Block airIDBright = LabStuffMain.brightAir;
        Block oxygenSealerID = LabStuffMain.oxygenSealer;
        LinkedList nextLayer = new LinkedList<BlockVec3>();
        World world = this.world;
        int side, bits;
