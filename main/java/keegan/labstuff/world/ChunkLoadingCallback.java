...
public class ChunkLoadingCallback implements LoadingCallback
{
    ...
    public static void loadConfig(File file)
    {
        ...
        {
            LSLog.severe("Problem loading chunkloading config (\"core.conf\")");
        }
        finally
        {
    ...
    public static void addToList(World world, int x, int y, int z, String playerName)
    {
        ...
        dimensionMap.put(LabStuffUtils.getDimensionID(world), chunkLoaders);
        ChunkLoadingCallback.chunkLoaderList.put(playerName, dimensionMap);
        ...
        if (saveDir != null)
        {
            ...
            if (!saveFile.exists())
            {
                try
                {
                    ...
                    {
                        LSLog.severe("Could not create chunk loader data file: " + saveFile.getAbsolutePath());
                    }
                }
                catch (IOException e)
                {
    ...
    private static File getSaveDir()
    {
        if (DimensionManager.getWorld(0) != null)
        {
            ...
            if (!saveDir.exists())
            {
                if (!saveDir.mkdirs())
                {
                    LSLog.severe("Could not create chunk loader save data folder: " + saveDir.getAbsolutePath());
                }
            }
