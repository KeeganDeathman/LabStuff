...
import keegan.labstuff.perlin.generator.*;
import keegan.labstuff.util.BlockVec3;
...
{
    final Block ASTEROID_STONE = LabStuffMain.asteroid;
    ...
    public ChunkProviderAsteroids(World par1World, long par2, boolean par4)
    {
        ...
        this.coreHandler.addBlock(new SpecialAsteroidBlock(Blocks.IRON_ORE, (byte) 12, 2, .13));
        ...
        this.shellHandler.addBlock(new SpecialAsteroidBlock(Blocks.PACKED_ICE, (byte) 0, 1, .15));
    }
    ...
    private void generateAsteroid(Random rand, int asteroidX, int asteroidY, int asteroidZ, int chunkX, int chunkZ, int size, ChunkPrimer primer, boolean flagDataOnly)
    {
        ...
        {
            isHollow = true;
            shell = new SpecialAsteroidBlock(Blocks.PACKED_ICE, (byte) 0, 1, .15);
        }
                ...
                for (int z = a.zMinArray; z < a.zMax; z++)
                {
                    for (int y = yMin; y < yMax; y++)
                    {
                        if (chunk.getBlockState(x - 1, y, z).getBlock() instanceof BlockAir && !(chunk.getBlockState(x, y, z).getBlock() instanceof BlockAir))
                        {
                            ...
                            chunk.setBlockState(new BlockPos(x - 1, y & 15, z), LabStuffMain.brightAir.getStateFromMeta(15 - count));
