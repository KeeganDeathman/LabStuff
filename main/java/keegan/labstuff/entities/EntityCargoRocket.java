...
public class EntityCargoRocket extends EntityAutoRocket implements IRocketType, IInventory, IWorldTransferCallback
{
    ...
    {
        return new ItemStack(LabStuffMain.rocketMars, 1, this.rocketType.getIndex() + 10);
    }
    ...
    protected void spawnParticles(boolean launched)
    {
        ...
        if (!this.isDead)
        {
            ...
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX + 0.2 - this.rand.nextDouble() / 10 + x1, y, this.posZ + 0.2 - this.rand.nextDouble() / 10 + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX - 0.2 + this.rand.nextDouble() / 10 + x1, y, this.posZ + 0.2 - this.rand.nextDouble() / 10 + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX - 0.2 + this.rand.nextDouble() / 10 + x1, y, this.posZ - 0.2 + this.rand.nextDouble() / 10 + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX + 0.2 - this.rand.nextDouble() / 10 + x1, y, this.posZ - 0.2 + this.rand.nextDouble() / 10 + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX + x1, y, this.posZ + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX + 0.2 + x1, y, this.posZ + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX - 0.2 + x1, y, this.posZ + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX + x1, y, this.posZ + 0.2D + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
            LabStuffMain.proxy.spawnParticle(this.getLaunched() ? "launchFlameLaunched" : "launchFlameIdle", new Vector3(this.posX + x1, y, this.posZ - 0.2D + z1), new Vector3(x1, y1, z1), new Object[] { riddenByEntity });
        }
    }
    ...
    public void onReachAtmosphere()
    {
        ...
        LSLog.debug("[Serverside] Cargo rocket reached space, heading to " + this.destinationFrequency);
        this.setTarget(true, this.destinationFrequency);

        if (this.targetVec != null)
        {
            LSLog.debug("Destination location = " + this.targetVec.toString());
            if (this.targetDimension != LabStuffUtils.getDimensionID(this.worldObj))
            {
                LSLog.debug("Destination is in different dimension: " + this.targetDimension);
                ...
                {
                    LSLog.debug("Loaded destination dimension " + this.targetDimension);
                    this.setPosition(this.targetVec.getX() + 0.5F, this.targetVec.getY() + 800, this.targetVec.getZ() + 0.5F);
                    Entity e = WorldUtil.transferEntityToDimension(this, this.targetDimension, (WorldServer) WorldUtil.getWorldForDimensionServer(targetDimension), false, null);

                    if (e instanceof EntityCargoRocket)
                    {
                        LSLog.debug("Cargo rocket arrived at destination dimension, going into landing mode.");
                    ...
                    {
                        LSLog.info("Error: failed to recreate the cargo rocket in landing mode on target planet.");
                        e.setDead();
                        this.setDead();
                    }
                    return;
                }
                LSLog.info("Error: the server failed to load the dimension the cargo rocket is supposed to land in. Destroying rocket!");
                this.setDead();
                return;
            }
            else
            {
                LSLog.debug("Cargo rocket going into landing mode in same destination.");
                this.setPosition(this.targetVec.getX() + 0.5F, this.targetVec.getY() + 800, this.targetVec.getZ() + 0.5F);
                this.landing = true;
                return;
            }
        ...
        {
            LSLog.info("Error: the cargo rocket failed to find a valid landing spot when it reached space.");
            this.setDead();
        }
    }
    ...
    public boolean processInitialInteract(EntityPlayer player, ItemStack stack, EnumHand hand)
    {
        ...
        {
            LabStuffUtils.openCargoRocketInventory((EntityPlayerMP) player, this);
        }
    ...
    public List<ItemStack> getItemsDropped(List<ItemStack> droppedItemList)
    {
        ...
        ItemStack rocket = new ItemStack(LabStuffMain.rocketMars, 1, this.rocketType.getIndex() + 10);
