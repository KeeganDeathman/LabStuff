...
import keegan.labstuff.world.IExitHeight;
...
public abstract class EntitySpaceshipBase extends Entity implements IPacketReceiver, IIgnoreShift, ITelemetry
{
    ...
    public void onUpdate()
    {
        ...
        if (!this.worldObj.isRemote)
        {
	        ...
	        else if (this.posY > (this.worldObj.provider instanceof IExitHeight ? ((IExitHeight) this.worldObj.provider).getYCoordinateToTeleport() : 1200) + 100)
	        {
                for (Entity e : this.getPassengers())
                {
                    ...
                    {
                        LSPlayerStats stats = LSPlayerStats.get(e);
        ...
        {
            LabStuffMain.packetPipeline.sendToDimension(new PacketDynamic(this), this.worldObj.provider.getDimension());
            // PacketDispatcher.sendPacketToAllInDimension(GCCorePacketManager.getPacket(LabStuffMain.CHANNELENTITIES,
    ...
    protected void failRocket()
    {
        ...
        {
            passenger.attackEntityFrom(DamageSourceLS.spaceshipCrash, (int) (4.0D * 20 + 1.0D));
        }
    ...
    {
        return LabStuffRegistry.getResouceLocationForDimension(this.worldObj.provider.getClass());
    }
    ...
    public void receiveData(int[] data, String[] str)
    {
		...
		str[0] = "";
		str[1] = (countdown == 400) ? LabStuffUtils.translate("gui.rocket.on_launchpad") : ((countdown > 0) ? LabStuffUtils.translate("gui.rocket.countdown") + ": " + countdown / 20 : LabStuffUtils.translate("gui.rocket.launched"));
		str[2] = LabStuffUtils.translate("gui.rocket.height") + ": " + data[1];
		str[3] = GameScreenText.makeSpeedString(data[2]);
		str[4] = LabStuffUtils.translate("gui.message.fuel.name") + ": " + data[3] + "%";
    }
