...
import keegan.labstuff.client.sounds.SoundUpdaterMiner;
    ...
    public void onUpdate()
    {
        ...
        if (this.ticksExisted % 10 == 0 || this.flagLink)
        {
            ...
            else
            {
                ...
                {
                    this.playerMP.addChatMessage(new TextComponentString(LabStuffUtils.translate("gui.message.astro_miner" + FAIL_BASEDESTROYED + ".fail")));
        ...
        if (this.playerMP == null)
        {
            ...
            this.motionY = 0;
            this.motionZ = 0;
            LabStuffMain.packetPipeline.sendToDimension(new PacketDynamic(this), LabStuffUtils.getDimensionID(this.worldObj));
            return;
        }
        ...
        case AISTATE_DOCKING:
            ...
            {
                LSLog.severe("AstroMiner missing base position: this is a bug.");
...
*/
    ...
    private void freeze(int i)
    {
        ...
        {
            this.playerMP.addChatMessage(new TextComponentString(LabStuffUtils.translate("gui.message.astro_miner" + i + ".fail")));
    ...
    private void atBase()
    {
        ...
        if (this.energyLevel >= MAXENERGY && !somethingTransferred && this.hasHoldSpace())
        {
            ...
            else
            {
                ...
                {
                    this.playerMP.addChatMessage(new TextComponentString(LabStuffUtils.translate("gui.message.astro_miner6.fail")));
                    this.givenFailMessage += 64;
                }
            }
        }
    ...
    private boolean findNextTarget(TileEntityMinerBase minerBase)
    {
        ...
        if (!this.minePoints.isEmpty() && this.pathBlockedCount < 2)
        {
            ...
            LSLog.debug("Still mining at: " + posTarget.toString() + " Remaining shafts: " + this.minePoints.size());
            return true;
        }
        ...
        LSLog.debug("Miner target: " + posTarget.toString());
    ...
    private boolean moveToTarget()
    {
        ...
        if (this.posTarget == null)
        {
            LSLog.severe("AstroMiner missing target: this is a bug.");
    ...
    private boolean doMining()
    {
        if (this.energyLevel < this.RETURNENERGY || this.inventoryDrops > this.RETURNDROPS || this.minePoints.size() == 0)
        {
            ...
            AIstate = AISTATE_RETURNING;
            this.pathBlockedCount = 0;
            LSLog.debug("Miner going home: " + this.posBase.toString() + " " + this.minePoints.size() + " shafts still to be mined");
            return true;
        }
        ...
        if (this.moveToPos(this.minePoints.getFirst(), false))
        {
            ...
            LSLog.debug("Miner mid mining: " + this.minePointCurrent.toString() + " " + this.minePoints.size() + " shafts still to be mined");
            return true;
        }
        return false;
    }
    ...
    private boolean prepareMove(int limit, int dist)
    {
        ...
        if (wayBarred)
        {
            if (this.playerMP != null)
            {
                this.playerMP.addChatMessage(new TextComponentString(LabStuffUtils.translate("gui.message.astro_miner1_a.fail") + " " + LabStuffUtils.translate(EntityAstroMiner.blockingBlock.toString())));
            }
            this.motionX = 0;
            this.motionY = 0;
            this.motionZ = 0;
    ...
    private boolean tryMineBlock(BlockPos pos)
    {
        ...
        if (b != LabStuffMain.fallenMeteor)
        {
    ...
    private boolean moveToPos(BlockVec3 pos, boolean reverse)
    {
        ...
        if (reverse != (this.baseFacing.getIndex() < 4))
        {
            if (this.posZ > pos.z + 0.0001D || this.posZ < pos.z - 0.0001D)
            {
                ...
                if (TEMPDEBUG)
                {
                    LSLog.debug("At " + posX + "," + posY + "," + posZ + "Moving Z to " + pos.toString() + (stopForTurn ? " : Stop for turn " + this.rotationPitch + "," + this.rotationYaw + " | " + this.targetPitch + "," + this.targetYaw : ""));
                }
            }
            else if (this.posY > pos.y - 0.9999D || this.posY < pos.y - 1.0001D)
            {
                ...
                if (TEMPDEBUG)
                {
                    LSLog.debug("At " + posX + "," + posY + "," + posZ + "Moving Y to " + pos.toString() + (stopForTurn ? " : Stop for turn " + this.rotationPitch + "," + this.rotationYaw + " | " + this.targetPitch + "," + this.targetYaw : ""));
                }
            }
            else if (this.posX > pos.x + 0.0001D || this.posX < pos.x - 0.0001D)
            {
                ...
                if (TEMPDEBUG)
                {
                    LSLog.debug("At " + posX + "," + posY + "," + posZ + "Moving X to " + pos.toString() + (stopForTurn ? " : Stop for turn " + this.rotationPitch + "," + this.rotationYaw + " | " + this.targetPitch + "," + this.targetYaw : ""));
                }
            }
            else
            {
...
//	    this.dataManager.set(this.currentDamage, Integer.valueOf(this.dataManager.get(this.currentDamage) * 5));
    ...
    public void setDead()
    {
        ...
        {
            LSPlayerStats stats = LSPlayerStats.get(this.playerMP);
    ...
    {
        ItemStack rocket = new ItemStack(LabStuffMain.astroMiner, 1, 0);
    ...
    public void receiveData(int[] data, String[] str)
    {
        ...
        str[4] = LabStuffUtils.translate("gui.energy_storage.desc.1") + ": " + energyPerCent + "%";
        switch (data[4])
        {
        case EntityAstroMiner.AISTATE_STUCK:
            str[0] = LabStuffUtils.translate("gui.message.no_energy.name");
            break;
        case EntityAstroMiner.AISTATE_ATBASE:
            str[0] = LabStuffUtils.translate("gui.miner.docked");
            break;
        case EntityAstroMiner.AISTATE_TRAVELLING:
            str[0] = LabStuffUtils.translate("gui.miner.travelling");
            break;
        case EntityAstroMiner.AISTATE_MINING:
            str[0] = LabStuffUtils.translate("gui.miner.mining");
            break;
        case EntityAstroMiner.AISTATE_RETURNING:
            str[0] = LabStuffUtils.translate("gui.miner.returning");
            break;
        case EntityAstroMiner.AISTATE_DOCKING:
            str[0] = LabStuffUtils.translate("gui.miner.docking");
            break;
        case EntityAstroMiner.AISTATE_OFFLINE:
            str[0] = LabStuffUtils.translate("gui.miner.offline");
            break;
        }
    }
    ...
    protected void readEntityFromNBT(NBTTagCompound nbt)
    {
        ...
        {
            System.out.println("[Labstuff] Please break and replace any AstroMiner placed in the world prior to build 3.0.11.317.");
