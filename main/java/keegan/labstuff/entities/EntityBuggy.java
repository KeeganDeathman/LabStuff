...
public class EntityBuggy extends Entity implements IInventory, IPacketReceiver, IDockable, IControllableEntity, IEntityFullSync
{
    ...
    {
        return new ItemStack(LabStuffMain.buggy, 1, this.buggyType);
    }
    ...
    public List<ItemStack> getItemsDropped()
    {
        ...
        ItemStack buggy = new ItemStack(LabStuffMain.buggy, 1, this.buggyType);
    ...
    public boolean processInitialInteract(EntityPlayer player, ItemStack stack, EnumHand hand)
    {
        if (this.worldObj.isRemote)
        {
            if (this.getPassengers().isEmpty())
            {
                player.addChatMessage(new TextComponentString(GameSettings.getKeyDisplayString(KeyHandlerClient.leftKey.getKeyCode()) + " / " + GameSettings.getKeyDisplayString(KeyHandlerClient.rightKey.getKeyCode()) + "  - " + LabStuffUtils.translate("gui.buggy.turn.name")));
                player.addChatMessage(new TextComponentString(GameSettings.getKeyDisplayString(KeyHandlerClient.accelerateKey.getKeyCode()) + "       - " + LabStuffUtils.translate("gui.buggy.accel.name")));
                player.addChatMessage(new TextComponentString(GameSettings.getKeyDisplayString(KeyHandlerClient.decelerateKey.getKeyCode()) + "       - " + LabStuffUtils.translate("gui.buggy.decel.name")));
                player.addChatMessage(new TextComponentString(GameSettings.getKeyDisplayString(KeyHandlerClient.openFuelGui.getKeyCode()) + "       - " + LabStuffUtils.translate("gui.buggy.inv.name")));
            }
    ...
    public boolean pressKey(int key)
    {
        ...
        {
            LabStuffMain.packetPipeline.sendToServer(new PacketSimple(PacketSimple.EnumSimplePacket.S_CONTROL_ENTITY, LabStuffUtils.getDimensionID(this.worldObj), new Object[] { key }));
            return true;
        }
    ...
    public int addFuel(FluidStack liquid, boolean doDrain)
    {
        ...
        {
            return FluidUtil.fillWithLSGas(this.buggyFuelTank, liquid, doDrain);
        }
