...
public class EntityMeteor extends Entity
{
    ...
    {
        LabStuffMain.proxy.spawnParticle("distanceSmoke", new Vector3(this.posX, this.posY + 1D + Math.random(), this.posZ), new Vector3(0.0D, 0.0D, 0.0D), new Object[] {});
        LabStuffMain.proxy.spawnParticle("distanceSmoke", new Vector3(this.posX + Math.random() / 2, this.posY + 1D + Math.random() / 2, this.posZ), new Vector3(0.0D, 0.0D, 0.0D), new Object[] {});
        LabStuffMain.proxy.spawnParticle("distanceSmoke", new Vector3(this.posX, this.posY + 1D + Math.random(), this.posZ + Math.random()), new Vector3(0.0D, 0.0D, 0.0D), new Object[] {});
        LabStuffMain.proxy.spawnParticle("distanceSmoke", new Vector3(this.posX - Math.random() / 2, this.posY + 1D + Math.random() / 2, this.posZ), new Vector3(0.0D, 0.0D, 0.0D), new Object[] {});
        LabStuffMain.proxy.spawnParticle("distanceSmoke", new Vector3(this.posX, this.posY + 1D + Math.random(), this.posZ - Math.random()), new Vector3(0.0D, 0.0D, 0.0D), new Object[] {});
    }
    ...
    protected void onImpact(RayTraceResult movingObjPos)
    {
        if (!this.worldObj.isRemote)
        {
            if (movingObjPos != null)
            {
                ...
                {
                    this.worldObj.setBlockState(movingObjPos.getBlockPos().up(), LabStuffMain.fallenMeteor.getDefaultState(), 3);
                }

                if (movingObjPos.entityHit != null)
                {
                    movingObjPos.entityHit.attackEntityFrom(EntityMeteor.causeMeteorDamage(this, this.shootingEntity), 6F);
                }
            }
