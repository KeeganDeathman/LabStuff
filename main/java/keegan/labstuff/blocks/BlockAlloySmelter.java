...
public class BlockAlloySmelter extends Block implements ITileEntityProvider {

	public BlockAlloySmelter(Material materialIn) {
		super(materialIn);
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack held, EnumFacing facing, float fx, float par8, float par9) {
		...
		if(FMLCommonHandler.instance().getEffectiveSide().isServer())
		{
			if (!player.isSneaking()) {
				player.openGui(LabStuffMain.instance, 21, world, pos.getX(), pos.getY(), pos.getZ());
				return true;
			}
		}
		return false;
	...
	{
		((AlloySmelter)world.getTileEntity(pos)).finishSmelting();
	}
