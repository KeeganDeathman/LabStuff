...
public class BlockIBM650Console extends Block implements ITileEntityProvider
{

	public static PropertyInteger RENDER = PropertyInteger.create("render", 0, 4);
	
	public BlockIBM650Console(Material materialIn) {
		super(materialIn);
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack held, EnumFacing facing, float fx, float par8, float par9) {
		if(!world.isRemote)
    	{
    		player.openGui(LabStuffMain.instance, 25, world, pos.getX(), pos.getY(), pos.getZ());
    		return true;
    	}
    	return false;
    }
